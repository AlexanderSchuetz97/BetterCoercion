...
import eu.aschuetz.bettercoercion.userdata.generic.JavaMethods;
...
import eu.aschuetz.bettercoercion.userdata.generic.JavaMethod;
...
public class Accessor {
    ...
    private final Map<LuaValue, AbstractFieldAccessor> fieldsTopDown = new HashMap<>();
    ...
    public Accessor(Map<LuaValue, MethodAccessor[]> methods, Map<LuaValue, AbstractFieldAccessor> fields, Map<LuaValue, MethodAccessor[]> constructors) {
        ...
        //order of fields has to be correct!
        for (Map.Entry<LuaValue, AbstractFieldAccessor> entry : fields.entrySet()) {
            ...
            LuaValue topDownKey = LuaString.valueOf("_" + entry.getValue().getFieldName());
            if (!fieldsTopDown.containsKey(topDownKey)) {
                fieldsTopDown.put(topDownKey, entry.getValue());
            }
        }
    ...
    public Map<LuaValue, AbstractFieldAccessor> getFieldsTopDown() {
        return fieldsTopDown;
    }
